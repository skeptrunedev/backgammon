...
export default function PipTrainingScreen() {
  ...
  const methodSelector = <label className="grid gap-1 text-xs text-muted-foreground">Method<select className="h-8 rounded-lg border bg-background px-2 text-sm text-foreground" value={methodChoice} onChange={(e) => { const v = e.target.value as MethodChoice; setMethodChoice(v); setLearnStep(0); remember(v); }}>{METHODS.map((m) => <option key={m.value} value={m.value}>{m.label}</option>)}{mode !== 'learn' && <option value="mixed">Mixed</option>}</select></label>;
