  ...
  return (
    <main className="mx-auto flex w-full max-w-5xl flex-col gap-5">
      ...
      {!loaded ? <div className="flex justify-center py-20"><LoaderCircleIcon className="size-5 animate-spin" /></div> : mode !== 'learn' && !positions.length ? <Card><CardContent className="py-12 text-center text-sm text-muted-foreground">Play an analyzed match to add personal positions. Learn mode is ready now.</CardContent></Card> : board && <div className={mode === 'learn' ? 'grid grid-cols-[minmax(0,1.45fr)_minmax(8rem,0.75fr)] items-start gap-2 sm:gap-4 md:grid-cols-[minmax(0,1.45fr)_minmax(17rem,0.75fr)]' : 'contents'}>
        ...
        {mode === 'learn' ? <Card><CardHeader className="p-3 pb-2 sm:p-6 sm:pb-3"><CardTitle className="text-sm sm:text-lg">{METHODS.find((m) => m.value === method)?.label}</CardTitle></CardHeader><CardContent className="grid gap-2 p-3 pt-0 sm:gap-3 sm:p-6 sm:pt-0">{methodSelector}<p className="hidden text-sm text-muted-foreground sm:block">{METHODS.find((m) => m.value === method)?.summary}</p>{breakdown?.stages.slice(0, learnStep).map((stage, i) => <div key={stage.label} className="rounded-lg bg-muted px-2 py-1.5 text-xs sm:px-3 sm:py-2 sm:text-sm"><div className="flex gap-1 sm:justify-between"><span>{i + 1}. {stage.label}</span><strong className="ml-auto tabular-nums">{stage.value > 0 ? '+' : ''}{stage.value}</strong></div><p className="mt-1 hidden text-xs text-muted-foreground sm:block">{METHODS.find((m) => m.value === method)?.steps[i]}</p></div>)}{learnStep < (breakdown?.stages.length ?? 0) ? <Button className="h-8 px-2 text-xs sm:h-9 sm:text-sm" onClick={() => setLearnStep((value) => value + 1)}>Show step {learnStep + 1}</Button> : <><p className="text-xs sm:text-sm">Result: <strong>{answer?.leader} leads by {answer?.margin} pips</strong> ({answer?.mine} to {answer?.theirs}).</p><Button className="h-8 px-2 text-xs sm:h-9 sm:text-sm" variant="outline" onClick={() => setLearnStep(0)}>Work through it again</Button></>}</CardContent></Card> : <Card><CardContent className="grid gap-4 pt-5">
