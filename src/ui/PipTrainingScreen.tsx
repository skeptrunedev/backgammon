...
export default function PipTrainingScreen() {
  ...
  return (
    <main className="mx-auto flex w-full max-w-4xl flex-col gap-5">
      ...
      <Card><CardContent className="flex flex-wrap gap-3 pt-5">
        <label className="grid gap-1 text-xs text-muted-foreground">Method<select className="h-8 rounded-lg border bg-background px-2 text-sm text-foreground" value={methodChoice} onChange={(e) => { const v = e.target.value as MethodChoice; setMethodChoice(v); setLearnStep(0); remember(v); }}>{METHODS.map((m) => <option key={m.value} value={m.value}>{m.label}</option>)}{mode !== 'learn' && <option value="mixed">Mixed</option>}</select></label>
        {mode !== 'learn' && <label className="grid gap-1 text-xs text-muted-foreground">Answer<select className="h-8 rounded-lg border bg-background px-2 text-sm text-foreground" value={answerMode} onChange={(e) => { const v = e.target.value as AnswerMode; setAnswerMode(v); remember(methodChoice, v); }}><option value="relative">Leader and margin</option><option value="exact">Both exact counts</option><option value="both">Exact counts, leader, and margin</option></select></label>}
      ...
      {!loaded ? <div className="flex justify-center py-20"><LoaderCircleIcon className="size-5 animate-spin" /></div> : mode !== 'learn' && !positions.length ? <Card><CardContent className="py-12 text-center text-sm text-muted-foreground">Play an analyzed match to add personal positions. Learn mode is ready now.</CardContent></Card> : board && <>
        <div className="mx-auto w-full max-w-3xl"><Board board={board} showDice={false} /></div>
        {mode === 'learn' ? <Card><CardHeader><CardTitle>{METHODS.find((m) => m.value === method)?.label}</CardTitle></CardHeader><CardContent className="grid gap-3"><p className="text-sm text-muted-foreground">{METHODS.find((m) => m.value === method)?.summary}</p>{breakdown?.stages.slice(0, learnStep).map((stage, i) => <div key={stage.label} className="rounded-lg bg-muted px-3 py-2 text-sm"><div className="flex justify-between"><span>{i + 1}. {stage.label}</span><strong className="tabular-nums">{stage.value > 0 ? '+' : ''}{stage.value}</strong></div><p className="mt-1 text-xs text-muted-foreground">{METHODS.find((m) => m.value === method)?.steps[i]}</p></div>)}{learnStep < (breakdown?.stages.length ?? 0) ? <Button onClick={() => setLearnStep((value) => value + 1)}>Show step {learnStep + 1}</Button> : <><p className="text-sm">Result: <strong>{answer?.leader} leads by {answer?.margin} pips</strong> ({answer?.mine} to {answer?.theirs}).</p><Button variant="outline" onClick={() => setLearnStep(0)}>Work through it again</Button></>}</CardContent></Card> : <Card><CardContent className="grid gap-4 pt-5">
